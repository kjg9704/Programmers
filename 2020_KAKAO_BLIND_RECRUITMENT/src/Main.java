	...
	static boolean check(ArrayList<Frame> list) {
		for (Frame frame : list) {//설치된 전체 프레임 구조
			...
			case board ://보일 때
				...
				if (list.contains(new Frame(frame.x,frame.y-1,0)) || list.contains(new Frame(frame.x+1,frame.y-1,0)) || (list.contains(new Frame(frame.x-1,frame.y,1)) && list.contains(new Frame(frame.x+1,frame.y,1)))) continue;
				else return false;
			}
		}
